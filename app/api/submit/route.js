...
export async function POST(request) {
    try {
        ...
        const { task, reason, name, firstname, lastname, email, phone, worth } = body || {};
        ...
        const { data, error } = await supabase
            ...
            .insert([{
                ...
                task, reason, name, firstname, lastname, email, phone, worth, status: 'New'
            }])
        ...
        try {
            const ideasResp = await fetch(`${origin}/api/ideas`, {
                ...
                body: JSON.stringify({ taskId, task, reason, worth })
            });
